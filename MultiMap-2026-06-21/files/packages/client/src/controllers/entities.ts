...
export default class EntitiesController {
    ...
    /**
     * Removes every entity except the local player (used when switching maps).
     */

    public clearExceptPlayer(player: Player): void {
        for (let instance in this.entities) {
            if (this.entities[instance] === player) continue;

            delete this.entities[instance];
        }
    }
