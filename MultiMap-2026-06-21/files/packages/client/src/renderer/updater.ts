...
export default class Updater {
    ...
    /**
     * Keeps movement transitions aligned with the active map tile size.
     */
    ...
    public update(): void {
        ...
        this.updateLights();
        this.game.menu.getOdinSkillsBar().tickCooldowns(this.game.player.odinSkills);
    }
    ...
    private updateEntities(): void {
        this.game.entities.forEachEntity((entity: Entity) => {
            ...
            if (entity.isProjectile()) {
                ...
                if (tDistance < 16 && entity.impactCallback) {
                    entity.impactCallback();
                    entity.impactCallback = undefined;
                }
            ...
            let stepDelta = entity.getPathStepDelta();
            ...
            if (isDiagonal) {
                this.startDiagonalMovement(entity, dx, dy);
                return;
            }
    ...
    private startDiagonalMovement(entity: Character, dx: number, dy: number): void {
        let startX = entity.x,
            startY = entity.y,
            endX = startX + dx * this.tileSize,
            endY = startY + dy * this.tileSize,
            duration = Math.round(entity.movementSpeed * Math.SQRT2);
    ...
    private updateKeyboard(): void {
        ...
        let { dx, dy } = player.getKeyboardDelta();
    ...
    private updateAnimations(): void {
        ...
        if (target && input.selectedCellVisible) target.update(this.game.time);

        if (this.game.odinSkillCaster.isTargetMode()) {
            this.game.odinSkillCaster.updateHover();
            this.game.odinSkillCaster.updateAnimation(this.game.time);
        }

        if (this.game.player.rangeTargetLocked) input.lockedTargetMarkerAnimation.update(this.game.time);
