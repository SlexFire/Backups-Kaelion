...
export default class Renderer {
    ...
    /** Fator entre o buffer interno do canvas e o tamanho exibido na tela (stretch CSS). */
    public displayScaleX = 1;
    public displayScaleY = 1;
    ...
    private loadSizes(): void {
        ...
        let displayWidth = this.camera.borderOffsetWidth,
            displayHeight = this.camera.borderOffsetHeight,
            canvasRoot = this.game.app.canvas;

        // Buffer interno (tiles) + exibição esticada para preencher toda a viewport.
        this.forEachCanvas((canvas: HTMLCanvasElement) => {
            canvas.width = this.canvasWidth;
            canvas.height = this.canvasHeight;
        });

        canvasRoot.style.width = `${displayWidth}px`;
        canvasRoot.style.height = `${displayHeight}px`;

        this.displayScaleX = displayWidth / this.canvasWidth;
        this.displayScaleY = displayHeight / this.canvasHeight;
    ...
    /**
     * Converte coordenadas do buffer interno do canvas para pixels CSS na tela.
     */
    public toDisplayCoords(x: number, y: number): { x: number; y: number } {
        return {
            x: x * this.displayScaleX,
            y: y * this.displayScaleY
        };
    }

    /**
     * Converte um comprimento do buffer interno para pixels CSS na tela.
     */
    public toDisplayScalar(value: number, axis: 'x' | 'y' = 'x'): number {
        return value * (axis === 'x' ? this.displayScaleX : this.displayScaleY);
    }
    ...
    private drawHoveringCell(): void {
        ...
        if (this.game.odinSkillCaster.isActive()) return;
    ...
    /**
     * Desenha o círculo de preview para skills Odin em modo de seleção de área.
     */
    private drawOdinSkillPreview(): void {
        this.entitiesContext.save();
        this.setCameraView(this.entitiesContext);

        if (this.game.odinSkillCaster.isActive())
            this.game.odinSkillCaster.drawPreview(
                this.entitiesContext,
                this.actualTileSize,
                this.game.time
            );

        if (this.game.odinSkillCaster.hasActiveArea())
            this.game.odinSkillCaster.drawActiveArea(this.entitiesContext, this.actualTileSize);
    ...
    private drawSelectedCell(): void {
        if (
            ...
            this.game.odinSkillCaster.isTargetMode()
        )
            return;
        ...
        if (targetData) {
            ...
            if (targetData.alpha !== undefined)
                this.entitiesContext.globalAlpha = targetData.alpha;
    ...
    /**
     * Locked ranged target marker (`/img/target.png`) above the mob's head with a light bounce.
     */
    private drawLockedTarget(): void {
        let { player } = this.game;

        if (
            !player.rangeTargetLocked ||
            !player.hasTarget() ||
            !player.isRanged() ||
            this.game.odinSkillCaster.isTargetMode()
        )
        ...
        if (target.dead || !target.isVisible() || !target.sprite?.loaded) return;

        let marker = this.game.sprites.get('locked-target');
        ...
        let entitySprite = target.sprite,
            z = this.camera.zoomFactor,
            scale = this.getRenderScale(target),
            entityW = entitySprite.width * scale,
            headX = target.x + entitySprite.offsetX + entityW / 2,
            headY = target.y + entitySprite.offsetY + target.offsetY,
            { frame } = this.game.input.lockedTargetMarkerAnimation,
            markerScale = 0.3,
            headGap = 2 * z,
            useSheet = this.game.input.lockedTargetMarkerAnimation.length > 1,
            sourceX = frame.x,
            sourceY = frame.y,
            sourceW = marker.width,
            sourceH = marker.height,
            markerW = marker.width * markerScale * z,
            markerH = marker.height * markerScale * z,
            bounce = Math.sin(this.game.time / 200) * 2 * z,
            drawX = ~~(headX * z - markerW / 2),
            drawY = ~~(headY * z - markerH - headGap + bounce);

        if (
            !useSheet &&
            marker.image instanceof HTMLImageElement &&
            marker.image.naturalWidth > 0
        ) {
            sourceX = 0;
            sourceY = 0;
            sourceW = marker.image.naturalWidth;
            sourceH = marker.image.naturalHeight;
            markerW = sourceW * markerScale * z;
            markerH = sourceH * markerScale * z;
            drawX = ~~(headX * z - markerW / 2);
            drawY = ~~(headY * z - markerH - headGap + bounce);
        }

        this.entitiesContext.save();
        this.entitiesContext.setTransform(1, 0, 0, 1, 0, 0);
        this.setCameraView(this.entitiesContext);

        this.entitiesContext.globalAlpha = 0.85 + Math.sin(this.game.time / 120) * 0.15;
    ...
    private drawCursor(): void {
        if (this.tablet || this.mobile) return;

        if (this.game.odinSkillCaster.isActive()) {
            this.drawOdinSelectionCursor();
            this.game.input.saveMouse();

            return;
        }

        if (this.game.input.isMouseRendered()) return;
    ...
    /**
     * Cursor pixel art de seleção Odin — substitui mão/alvo vanilla durante o cast.
     */
    private drawOdinSelectionCursor(): void {
        let { mouse } = this.game.input;

        if (!mouse) return;

        let caster = this.game.odinSkillCaster,
            sprite = this.game.sprites.get('cursors/odin_target'),
            canConfirm = caster.canConfirmAtCursor(),
            size = this.actualTileSize,
            cx = mouse.x + size / 2,
            cy = mouse.y + size / 2;
        ...
        if (sprite?.loaded) {
            let pulse = 0.97 + Math.sin(this.game.time / 110) * 0.03,
                drawSize = Math.round(size * pulse),
                dx = Math.round(cx - drawSize / 2),
                dy = Math.round(cy - drawSize / 2);

            this.cursorContext.imageSmoothingEnabled = false;
            this.cursorContext.globalAlpha = canConfirm ? 1 : 0.55;
            ...
            let colors = caster.getSelectionColors(),
                stroke = canConfirm ? colors.primary : colors.invalid;

            this.cursorContext.strokeStyle = stroke;
            this.cursorContext.lineWidth = 2;
            this.cursorContext.beginPath();
            this.cursorContext.arc(cx, cy, size * 0.34, 0, Math.PI * 2);
    ...
    /**
     * Visual scale for rendering (sprite sheet crop unchanged). Uses mob `customScale`
     * when set, otherwise per-sprite `renderScale` from sprites.json.
     */

    private getRenderScale(entity: Entity): number {
        if (entity.customScale) return entity.customScale;

        return entity.sprite?.renderScale ?? 1;
    }
    ...
    private drawEntity(entity: Entity): void {
        ...
        // Visual scale (e.g. tall skins drawn at half size without changing the sheet crop).
        ...
        if (entity instanceof Character && entity.hasActiveEffect())
            this.drawEffects(entity, true, context);
        ...
        context.drawImage(
            ...
            entity.isProjectile()
                ? entity.animation!.frame.index * entity.sprite.width
                : frame!.x,
            entity.isProjectile()
                ? entity.animation!.row * entity.sprite.height
                : frame!.y,
            entity.sprite.width,
        ...
        if (!this.game.overlays.hasOverlay()) {
            let isLocalPlayer = this.game.player.instance === entity.instance,
                useScreenCenteredName = isLocalPlayer && this.camera.isCentered();

            if (useScreenCenteredName) this.drawPlayerName(entity as Player);
    ...
    private drawEntityFore(entity: Entity): void {
        ...
        if (entity.hasActiveEffect()) this.drawEffects(entity, false);
    }
    ...
    private drawEffects(
        ...
        for (let key of character.statusEffects) {
            ...
            if ((effect.behindBody ?? false) !== behind) continue;
            ...
            let { animation } = effect,
                ...
                dw = sprite.width,
                dh = sprite.height,
                drawScale = effect.drawScale ?? 1;

            if (drawScale !== 1) {
                dw = Math.round(sprite.width * drawScale);
                dh = Math.round(sprite.height * drawScale);
            }

            if (effect.anchorFeet || effect.anchorBody) {
                let body = character.sprite,
                    renderScale = this.getRenderScale(character),
                    bodyW = body.width * renderScale,
                    bodyH = body.height * renderScale,
                    centerX = body.offsetX + bodyW / 2;

                if (effect.scaleToBody) {
                    dw = Math.max(sprite.width, Math.ceil(bodyW * 1.4));
                    dh = Math.max(sprite.height, Math.ceil(bodyH * 1.6));
                }

                if (effect.anchorFeet) {
                    let feetY = body.offsetY + bodyH + character.offsetY;

                    dx = centerX - dw / 2 + (effect.anchorOffsetX ?? 0);
                    dy = feetY - dh + (effect.anchorOffsetY ?? 0);
                } else {
                    let anchorRatio = effect.bodyAnchorRatio ?? 0.42,
                        centerY = body.offsetY + bodyH * anchorRatio + character.offsetY;

                    dx = centerX - dw / 2 + (effect.anchorOffsetX ?? 0);
                    dy = centerY - dh / 2 + (effect.anchorOffsetY ?? 0);
                }
            }
    ...
    /**
     * Draws a special crown beside the player's name.
     ...
     * @param name The player's name used to align the crown to its start.
     */

    private drawCrown(key: string, x: number, y: number, name: string): void {
        let crown = this.getCrown(key);

        if (!crown) return;

        let displayWidth = crown.width * 2,
            displayHeight = crown.height * 2,
            gap = 2,
            nameWidth = this.measureTextWidth(name);
        ...
        let crownX = x - nameWidth / 2 - displayWidth - gap * this.camera.zoomFactor,
            crownY = y - displayHeight * 0.85;
     ...
     * @param name The player's name used to position the crown beside it.
     */

    private drawPlayerCrown(key: string, name: string): void {
        let crown = this.getCrown(key);
        ...
        let displayWidth = crown.width * 2,
            displayHeight = crown.height * 2,
            gap = 2 * this.camera.zoomFactor,
            nameWidth = this.measureTextWidth(name),
            screenX = this.camera.borderOffsetWidth / 2 + 8 * this.camera.zoomFactor,
            nameOffset = this.drawLevels ? 22 : 10,
            screenY = this.camera.borderOffsetHeight / 2 - nameOffset * this.camera.zoomFactor,
            crownX = screenX - nameWidth / 2 - displayWidth - gap,
            crownY = screenY - displayHeight * 0.85;
    ...
    private drawName(entity: Character | Item): void {
        ...
        // Handle additional player rank logic.
        if (entity.isPlayer() && entity.hasRank()) colour = Modules.RankColours[entity.rank];
        ...
        // Draw guild name above the player name.
        if (drawNames && entity.isPlayer()) {
            let player = entity as Player;

            if (player.guildName) {
                let guildY = drawLevels ? nameY - 12 : nameY - 10;

                this.drawText(player.guildName, x, guildY, true, true, 'rgba(140, 200, 255, 1)');
            }
        }
        ...
        if (drawNames && entity.isPlayer() && entity.hasCrown())
            this.drawCrown(entity.getCrownKey(), x, nameY, entity.name);
    ...
    private drawPlayerName(entity: Player): void {
        let nameOffset = this.drawLevels ? 22 : 10,
            guildOffset = entity.guildName ? 12 : 0;
        ...
        if (this.drawNames) {
            if (entity.guildName)
                this.drawText(
                    ...
                    this.camera.borderOffsetHeight / 2 -
                        (nameOffset + guildOffset) * this.camera.zoomFactor,
                    true,
                    false,
                    'rgba(140, 200, 255, 1)'
                );
            ...
            if (entity.hasCrown()) this.drawPlayerCrown(entity.getCrownKey(), entity.name);
        }
    ...
    /**
     * Measures the width of a string using the name tag font.
     * @param text The string we are measuring.
     * @param fontSize The font size to use for measurement.
     */

    private measureTextWidth(text: string, fontSize: number = this.fontSize): number {
        this.textContext.save();

        this.textContext.font = `${fontSize}px Quaver`;

        let width = this.textContext.measureText(text).width;
    ...
    protected getCrown(key: string): Sprite | undefined {
        switch (key) {
            ...
            case 'crown-adm': {
                return this.game.sprites.get('crowns/adm');
            }

            case 'crown-tier0': {
                return this.game.sprites.get('crowns/tier0');
            }
