...
export default class Camera {
    ...
    public update(): void {
        ...
        this.gridWidth = Math.max(
            1,
            Math.ceil(this.borderOffsetWidth / (this.tileSize * this.zoomFactor))
        );
        this.gridHeight = Math.max(
            1,
            Math.ceil(this.borderOffsetHeight / (this.tileSize * this.zoomFactor))
        );
    ...
    /**
     * Updates map dimensions after switching to another map instance.
     */

    public setMapSize(width: number, height: number, tileSize: number): void {
        this.width = width;
        this.height = height;
    ...
    /**
     * Instanced/small maps show a large fraction of the world on screen. The default
     * edge clamping locks the camera early and desyncs centred sprites from HUD text.
     */
    public isCompactMap(): boolean {
        return this.gridWidth >= this.width / 4 || this.gridHeight >= this.height / 4;
    }
