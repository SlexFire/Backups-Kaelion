...
import OdinSkillCaster from './controllers/odin_skill_caster';
...
export default class Game {
    ...
    public odinSkillCaster: OdinSkillCaster;
    ...
    public constructor(public app: App) {
        ...
        this.odinSkillCaster = new OdinSkillCaster(this);
        ...
        this.odinSkillCaster.onUse((key, x, y, targetInstance) =>
            this.menu.sendOdinSkillUse(key, x, y, targetInstance)
        );
    ...
    public getAttackApproachTile(
        character: Character,
        target: Entity
    ): { gridX: number; gridY: number } | null {
        if (character.attackRange > 1) return null;

        let best: { gridX: number; gridY: number; length: number } | null = null;

        for (let dx = -1; dx <= 1; dx++)
            for (let dy = -1; dy <= 1; dy++) {
                if (dx === 0 && dy === 0) continue;

                let gridX = target.gridX + dx,
                    gridY = target.gridY + dy;

                if (this.map.isOutOfBounds(gridX, gridY) || this.map.isColliding(gridX, gridY))
                    continue;

                let path = this.findPath(character, gridX, gridY);

                if (path.length < 2) continue;

                if (!best || path.length < best.length)
                    best = { gridX, gridY, length: path.length };
            }

        return best ? { gridX: best.gridX, gridY: best.gridY } : null;
    }

    public getRangedApproachTile(
        character: Character,
        target: Entity
    ): { gridX: number; gridY: number } | null {
        if (character.attackRange <= 1) return null;

        let range = character.attackRange,
            best: { gridX: number; gridY: number; length: number } | null = null;
