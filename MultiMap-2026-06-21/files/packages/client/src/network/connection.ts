...
import type { SerializedOdinSkill, OdinSkillData } from '@kaetram/common/network/impl/odinSkill';
...
export default class Connection {
    ...
    public constructor(private game: Game) {
        ...
        this.messages.onTask(this.handleTask.bind(this));
        ...
        this.messages.onBattlePass(this.handleBattlePass.bind(this));
        this.messages.onOdinSkill(this.handleOdinSkill.bind(this));
        this.messages.onCosmo(this.handleCosmo.bind(this));
    }
    ...
    private handleDespawn(info: DespawnPacketData): void {
        ...
        // Efeitos no chão (VFX de skills Odin, lava, etc.) — sem animação de morte.
        if (entity.isEffect()) return this.entities.removeEntity(entity);
    ...
    private handleCombat(opcode: Opcodes.Combat, info: CombatPacketData): void {
        ...
        // Skill icons: attacker feedback when you hit, damage-type feedback when you get hit.
        let hitSkills =
            currentPlayerAttacker || currentPlayerTarget ? info.hit.skills ?? [] : [];
    ...
    private handlePoints(info: PointsPacketData): void {
        ...
        if (info.mana !== undefined) character.setMana(info.mana, info.maxMana);

        if (info.hitPoints !== undefined) {
            character.setHitPoints(info.hitPoints, info.maxHitPoints);
            this.input.hud.updateCallback?.(info.instance, info.hitPoints);
        }
    }

    private handleBattlePass(info: BattlePassPacketData): void {
        let battlePass = this.menu.getBattlePass();

        if (info.exp !== undefined) battlePass.setExp(info.exp);

        if (info.premium !== undefined) battlePass.setPremium(info.premium);
    }
    ...
    private handleCommand(info: CommandPacketData): void {
        ...
        switch (info.command) {
            ...
            case 'autoatk': {
                this.game.player.autoAtkEnabled = info.enabled ?? false;

                if (!this.game.player.autoAtkEnabled) {
                    this.game.player.autoAtkTargetLocked = false;
                    this.game.player.autoAtkLockInstance = '';
                    this.game.player.rangeTargetLocked = false;
    ...
    /**
     * Handler para pacotes de skills Odin.
     */
    private handleOdinSkill(opcode: Opcodes.OdinSkill, info: OdinSkillPacketData): void {
        switch (opcode) {
            case Opcodes.OdinSkill.Batch: {
                this.game.player.loadOdinSkills((info as SerializedOdinSkill).skills);
                this.menu.getOdinSkillsBar().applyHotbarFromCosmos(this.game.player);
                break;
            }

            case Opcodes.OdinSkill.Add:
            case Opcodes.OdinSkill.Update:
            case Opcodes.OdinSkill.State:
            case Opcodes.OdinSkill.Cooldown: {
                this.game.player.setOdinSkill(info as OdinSkillData);
                break;
            }

            case Opcodes.OdinSkill.Cast: {
                let cast = info as {
                    key: string;
                    x?: number;
                    y?: number;
                ...
                this.menu.getOdinSkillsBar().refreshAll(this.game.player.odinSkills);

                if (cast.key) {
                    let skill = this.game.player.getOdinSkill(cast.key);

                    if (skill) this.menu.getOdinSkillsBar().updateSkill(skill);

                    if (
                        cast.key === 'volcanic_meteors' &&
                        cast.x !== undefined &&
                        cast.y !== undefined
                    ) {
                        let skillData = this.game.player.getOdinSkill('volcanic_meteors'),
                            radius = cast.areaRadius ?? skillData?.areaRadius ?? 2,
                            durationMs = (skillData?.areaDuration ?? 5) * 1000;

                        this.game.odinSkillCaster.activateAreaEffect(
                            cast.key,
                            cast.x,
                            cast.y,
                            radius,
    ...
    private handleCosmo(opcode: Opcodes.Cosmo, info: CosmoPacketData): void {
        switch (opcode) {
            case Opcodes.Cosmo.Batch: {
                this.game.player.cosmos.load(info as import('@kaetram/common/network/impl/cosmo').SerializedCosmoBag);
                this.menu.getCosmoBag().synchronize();
                this.menu.getOdinSkillsBar().applyHotbarFromCosmos(this.game.player);
                break;
            }

            case Opcodes.Cosmo.Add: {
                this.game.player.cosmos.add(info as import('@kaetram/common/network/impl/cosmo').CosmoInstanceData);
                this.menu.getCosmoBag().synchronize();
                break;
            }

            case Opcodes.Cosmo.Update: {
                this.game.player.cosmos.update(info as import('@kaetram/common/network/impl/cosmo').CosmoInstanceData);
                this.menu.getCosmoBag().synchronize();
                this.menu.getOdinSkillsBar().applyHotbarFromCosmos(this.game.player);
                break;
            }

            case Opcodes.Cosmo.Remove: {
                let removed = info as { instanceId: string };

                this.game.player.cosmos.remove(removed.instanceId);
                this.menu.getCosmoBag().synchronize();
                this.menu.getOdinSkillsBar().applyHotbarFromCosmos(this.game.player);
                break;
            }
    ...
    private handleTask(opcode: Opcodes.Task, info: TaskPacketData): void {
        switch (opcode) {
            case Opcodes.Task.Batch: {
                this.game.player.loadTasks(info.tasks!);
                break;
            }

            case Opcodes.Task.Progress: {
                this.game.player.setTask(
                    info.key!,
                    info.stage!,
                    info.name!,
                    info.description!,
        ...
        this.menu.getTasks().handle(opcode, info.key);
    }
    ...
    private handleHeal(info: HealPacketData): void {
        ...
        switch (info.type) {
            case 'hitpoints': {
                ...
                if (!info.skipVanillaEffect) character.addEffect(Modules.Effects.Healing);
    ...
    private handleDeath(): void {
        ...
        // Show the respawn screen immediately (intro is hidden during gameplay).
    ...
    private handleStore(opcode: Opcodes.Store, info: StorePacketData): void {
        let isCashShop = info.key === 'cosmetics';

        switch (opcode) {
            case Opcodes.Store.Open: {
                if (isCashShop) return this.menu.getCashShop().show(info);

                return this.menu.getStore().show(info);
            }

            case Opcodes.Store.Update: {
                if (isCashShop) return this.menu.getCashShop().update(info);
