...
export default class ProcessMap {
    ...
    /**
     * Resolves a valid player spawn for the exported map. Priority:
     * 1. Object layer named `playerspawn` (or `spawn`) in the Tiled map
     * 2. Global SPAWN_POINT / TUTORIAL_SPAWN_POINT if within bounds and walkable
     * 3. First walkable tile found from the map centre outward
     */

    private resolveSpawn(): void {
        let spawn = this.getSpawnFromAreas();

        if (!spawn) {
            let configSpawn = Utils.getPositionFromString(Modules.Constants.SPAWN_POINT);

            if (this.isValidSpawn(configSpawn.x, configSpawn.y)) spawn = configSpawn;
        }

        if (!spawn) {
            spawn = this.findFallbackSpawn();

            log.warning(
                `No valid spawn configured for this map. Assigned fallback spawn at ${spawn.x},${spawn.y}.`
            );
        } else log.notice(`Player spawn set to ${spawn.x},${spawn.y}.`);

        this.map.spawn = spawn;

        let tutorialSpawn = Utils.getPositionFromString(Modules.Constants.TUTORIAL_SPAWN_POINT);

        this.map.tutorialSpawn = this.isValidSpawn(tutorialSpawn.x, tutorialSpawn.y)
            ? tutorialSpawn
            : { ...spawn };
    }

    /**
     * Reads spawn position from Tiled object layers (`playerspawn`, `spawn`, etc.).
     */

    private getSpawnFromAreas(): Position | undefined {
        for (let name of ['playerspawn', 'playerSpawn', 'spawn']) {
            let areas = this.map.areas[name];
            ...
            for (let area of areas) {
                let candidates: Position[] = [
                    { x: area.x, y: area.y },
                    {
                        x: Math.floor(area.x + (area.width || 0) / 2),
                        y: Math.floor(area.y + (area.height || 0) / 2)
                    }
                ];

                for (let candidate of candidates)
                    if (this.isValidSpawn(candidate.x, candidate.y)) return candidate;
            }
        }
    }

    /**
     * Whether a grid coordinate is inside the map and not colliding.
     */

    private isValidSpawn(x: number, y: number): boolean {
        let { width, height, data, collisions } = this.map;
        ...
        return !this.isCollidingTile(tile, collisions);
    }

    private isCollidingTile(tile: number | number[], collisions: number[]): boolean {
        let check = (tileId: number) => collisions.includes(tileId);

        if (Array.isArray(tile)) return tile.some(check);

        return check(tile);
    }

    /**
     * Scans outward from the map centre for the first walkable tile.
     */

    private findFallbackSpawn(): Position {
        let { width, height } = this.map,
            cx = Math.floor(width / 2),
            cy = Math.floor(height / 2);

        if (this.isValidSpawn(cx, cy)) return { x: cx, y: cy };

        for (let radius = 1; radius < Math.max(width, height); radius++)
            for (let x = cx - radius; x <= cx + radius; x++)
                for (let y = cy - radius; y <= cy + radius; y++)
                    if (this.isValidSpawn(x, y)) return { x, y };

        log.error('Could not find any walkable tile for spawn; defaulting to 0,0.');
    ...
    /**
     * Returns the resolved player spawn for registry updates.
     */

    public getSpawn(): Position | undefined {
