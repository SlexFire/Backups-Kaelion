...
import { applyPendingKcoinCredits } from '../../../../network/mercadopago-service';
...
import type { TaskPacketData } from '@kaetram/common/network/impl/task';
...
export default class Incoming {
        ...
        this.connection.onMessage(([packet, message]) => {
            ...
            try {
                switch (packet) {
                    ...
                    case Packets.OdinSkill: {
                        return this.handleOdinSkill(message);
                    }
                    case Packets.Cosmo: {
                        return this.handleCosmo(message);
                    }
    ...
    /**
     * Handles the login process for Kaelion.
    ...
    private handleLogin(data: LoginPacket): void {
        ...
        if (username) {
            ...
            if (config.skipDatabase) {
                let info = Creator.serialize(this.player);

                info.mapId = this.world.maps.getDefaultId();
                info.x = -1;
                info.y = -1;
                info.regionsLoaded = [];
                info.mapVersion = -1;

                this.player.load(info);
                return;
            }
        }
    ...
    private handleReady(data: ReadyPacket): void {
        ...
        void applyPendingKcoinCredits(this.player);
    ...
    private handleMovement(data: MovementPacket): void {
        ...
        if (this.player.map.isOutOfBounds(requestX!, requestY!)) return;
        if (this.player.map.isOutOfBounds(playerX!, playerY!)) return;
    ...
    /**
     * Handles incoming task (achievement) actions from the client.
     * @param packet Contains the key of the task we are claiming.
     */

    private handleTask(packet: TaskPacketData): void {
        if (!packet.key) return;

        this.player.tasks.claimReward(packet.key);
    }
    ...
    /**
     * Processa uso de skills Odin vindas do cliente.
     */
    private handleCosmo(packet: CosmoPacket): void {
        if (packet.hotbarSlot !== undefined) {
            packet.hotbarSlot = Utils.sanitizeNumber(packet.hotbarSlot);

            if (packet.hotbarSlot < 0 || packet.hotbarSlot > 4) packet.hotbarSlot = undefined;
        }

        switch (packet.opcode) {
            case Opcodes.Cosmo.Equip: {
                if (!packet.instanceId || packet.hotbarSlot === undefined) return;

                this.player.cosmos.equip(packet.instanceId, packet.hotbarSlot);
                return;
            }

            case Opcodes.Cosmo.Unequip: {
                if (packet.hotbarSlot === undefined) return;

                this.player.cosmos.unequip(packet.hotbarSlot);
                return;
            }

            case Opcodes.Cosmo.Consume: {
                if (!packet.targetInstanceId || !packet.sourceInstanceId) return;

                this.player.cosmos.consumeExp(packet.targetInstanceId, packet.sourceInstanceId);
                return;
            }
        }
    }

    private handleOdinSkill(packet: OdinSkillPacket): void {
        if (packet.x !== undefined) packet.x = Utils.sanitizeNumber(packet.x);
        if (packet.y !== undefined) packet.y = Utils.sanitizeNumber(packet.y);
        if (packet.targetInstance !== undefined && typeof packet.targetInstance !== 'string')
            packet.targetInstance = undefined;
        if (packet.slotIndex !== undefined) {
            packet.slotIndex = Utils.sanitizeNumber(packet.slotIndex);

            if (packet.slotIndex < 0 || packet.slotIndex > 4) packet.slotIndex = undefined;
        }

        switch (packet.opcode) {
            case Opcodes.OdinSkill.Use: {
                return this.player.odinSkills.use(
                    packet.key,
                    packet.x,
                    packet.y,
                    packet.slotIndex,
                    packet.targetInstance
                );
            }
        }
    }
    ...
    private handleStore(data: StorePacket): void {
        ...
        switch (data.opcode) {
            case Opcodes.Store.Open: {
                return this.world.stores.openByKey(this.player, data.key);
            }

            case Opcodes.Store.Close: {
                return this.world.stores.close(this.player);
            }
        }
