...
export default abstract class Character extends Entity {
    ...
    private handleAoE(damage: number, attacker?: Character, range = 1): void {
        this.applyAoE(damage, attacker, range);
    }

    /**
     * Applies splash/AoE damage to characters near `this` entity's position.
     * @param excludeInstance Primary target instance to skip (already took direct damage).
     */
    ...
    public applyAoE(
        ...
        this.forEachNearbyCharacter((character: Character) => {
            if (excludeInstance && character.instance === excludeInstance) return;

            let distance = this.getDistance(character) + 1,
                splashDamage = Math.floor(damage / distance),
                hit = new Hit(
                    Modules.Hits.Normal,
                    splashDamage,
                    false,
            ...
            character.hit(splashDamage, attacker);
        }, range);
    }
    ...
    public findAdjacentTile(): void {
        if (!this.map.isColliding(this.x + 1, this.y)) this.setPosition(this.x + 1, this.y);
     ...
     * @param aoeFromAttacker When true, splash radiates from the attacker (melee weapons).
     */
    ...
    public hit(
        ...
        if (aoe) {
            if (aoeFromAttacker && attacker)
                attacker.applyAoE(damage, attacker, aoe, this.instance);
            else this.handleAoE(damage, attacker, aoe);
        }
    ...
    public isNearTarget(): boolean {
        ...
        if (this.isRanged()) {
            ...
            if (this.isPlayer() && this.moving) range++;

            return (
                this.getDistance(this.target!) <= range &&
    ...
    public override setPosition(x: number, y: number, withTeleport = false): void {
        ...
        this.map.grids.updateEntity(this);
    ...
    /**
     * Orienta o personagem em direção a outro alvo (eixo dominante X ou Y).
     */
    public lookAt(target: Character): void {
        if (this.x < target.x) this.setOrientation(Modules.Orientation.Right);
        else if (this.x > target.x) this.setOrientation(Modules.Orientation.Left);
        else if (this.y < target.y) this.setOrientation(Modules.Orientation.Down);
        else if (this.y > target.y) this.setOrientation(Modules.Orientation.Up);
    }
