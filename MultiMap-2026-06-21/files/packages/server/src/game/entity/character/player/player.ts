...
import OdinSkills from './odin_skills';
...
import {
    ...
    VIP_MAX_TIER,
    getVipChatColour,
    getVipExperienceMultiplier,
    canUseVipAutoAtk,
    getVipTitle,
    isVipRank,
    rankToVipTier,
    vipTierToRank
} from '@kaetram/common/config/vip';
import Utils from '@kaetram/common/util/utils';
import {
    BATTLE_PASS_PROGRESS,
    getBattlePassClaimKey,
    getBattlePassLevelFromExp,
    getLegacyBattlePassClaimKey,
    hasBattlePassUnlockedLevel
} from '@kaetram/common/config/battlepass';
...
import type { BattlePassTrack } from '@kaetram/common/config/battlepass';
...
export default class Player extends Character {
    ...
    public vipExpiresAt = 0;
    public vipTier = 0;
    public vipTierPeriodStart = 0;

    /** VIP /autoatk: auto-target mobs when entering weapon range while moving freely. */
    public autoAtkEnabled = false;

    /** Sticky /autoatk lock — only released when leaving that mob's range. */
    public autoAtkLockInstance = '';
    ...
    /** Recompensas do passe de batalha ja coletadas nesta conta. */
    public battlePassClaims: string[] = [];

    public battlePassExp = 0;

    public battlePassPremium = false;
    ...
    public constructor(
        ...
        this.odinSkills = new OdinSkills(this);
    ...
    public async load(data: PlayerInfo): Promise<void> {
        ...
        this.vipExpiresAt = data.vipExpiresAt || 0;
        this.vipTier = data.vipTier ?? 0;
        this.vipTierPeriodStart = data.vipTierPeriodStart || 0;
        this.ban = data.ban;
        this.jail = data.jail;
        this.mute = data.mute;
        ...
        this.loadAbilities();
        this.loadCosmos();
        this.odinSkills.initializeDefaults();
    ...
    public async loadTasks(): Promise<void> {
        let data = await this.database.loader?.loadTasks(this);

        this.tasks.load(
            data ?? {
                tasks: [],
                activeTasks: [],
                taskResets: { daily: 0, weekly: 0, monthly: 0 }
            }
        );
    }
    ...
    /**
     * Loads battle pass progress for the account (email) from the database.
     */

    public loadBattlePass(): Promise<void> {
        return new Promise((resolve) => {
            if (!this.database.loader) {
                resolve();

                return;
            }

            this.database.loader.loadBattlePass(this, (data) => {
                this.battlePassExp = data.battlePassExp;
                this.battlePassPremium = data.battlePassPremium;
                this.battlePassClaims = data.battlePassClaims;
    ...
    public getBattlePassLevel(): number {
        return getBattlePassLevelFromExp(this.battlePassExp);
    }

    public hasBattlePassPremium(): boolean {
        return this.battlePassPremium;
    }

    public unlockBattlePassPremium(): void {
        if (this.battlePassPremium) return;

        this.battlePassPremium = true;

        if (!this.isGuest) this.database.creator?.saveBattlePass(this);

        this.send(new BattlePassPacket({ premium: true }));

        this.notify('Trilha premium do Passe de Batalha liberada!');
    }

    public canClaimBattlePassReward(track: BattlePassTrack, level: number): boolean {
        if (!hasBattlePassUnlockedLevel(this.getBattlePassLevel(), level)) return false;

        if (track === 'paid' && !this.battlePassPremium) return false;

        return true;
    }

    /**
     * Adds battle pass XP (e.g. +1 per mob kill) and syncs the client.
     */

    public addBattlePassExp(amount = BATTLE_PASS_PROGRESS.expPerMobKill): void {
        let previousLevel = this.getBattlePassLevel();

        this.battlePassExp += amount;

        let currentLevel = this.getBattlePassLevel();

        if (!this.isGuest) this.database.creator?.saveBattlePass(this);

        this.send(
            new BattlePassPacket({
                exp: this.battlePassExp,
                premium: this.battlePassPremium
            })
        );

        if (currentLevel > previousLevel) this.notify(`Passe de Batalha: Lv - ${currentLevel}!`);
    }
    ...
    public loadCosmos(): void {
        this.database.loader?.loadCosmos(this, this.cosmos.load.bind(this.cosmos));
    }
    ...
    public intro(): void {
        ...
        // Update the player's timeout based on their rank or VIP tier.
        if (this.rank !== Modules.Ranks.None || this.isVip())
    ...
    public override heal(
        amount = 1,
        type: Modules.HealTypes = 'passive',
        skipVanillaEffect = false
    ): void {
        switch (type) {
            case 'passive': {
            ...
            case 'mana': {
                ...
                this.sendToRegions(
                    new HealPacket({
                        ...
                        type,
                        amount,
                        skipVanillaEffect: skipVanillaEffect || undefined
                    })
                );
                break;
            }
    ...
    /**
     * Valida se o jogador pode atacar o alvo (exposto para skills externas ao Character).
     */
    public canAttackTarget(target: Character): boolean {
    ...
    public handleTargetAttack(instance: string, x?: number, y?: number): void {
        ...
        if (!this.canAttackTarget(target)) return;
        ...
        if (this.autoAtkEnabled && target.isMob()) {
            this.autoAtkLockInstance = instance;
        }
    ...
    public handleExperience(damage: number): void {
        ...
        let experience =
                damage * this.world.getExperiencePerHit() * this.getVipExperienceMultiplier(),
        ...
        switch (weapon.attackStyle) {
            case Modules.AttackStyle.Melee: {
                this.skills
            ...
            default: {
                this.skills
                    .get(Modules.Skills.Accuracy)
                    .addExperience(Math.ceil(experience * 0.3), false);
                this.skills
                    .get(Modules.Skills.Strength)
                    .addExperience(Math.ceil(experience * 0.3), false);
                this.skills
                    .get(Modules.Skills.Defense)
                    .addExperience(Math.ceil(experience * 0.15), false);
                break;
            }
        }
    }
    ...
    public handleMovementRequest(x: number, y: number, target: string): void {
        if (!target) {
            if (this.autoAtkEnabled) {
                this.autoAtkLockInstance = '';
                this.target = undefined;
                this.combat.stop();
            } else {
                this.target = undefined;
            }
        } else if (target !== this.target?.instance) {
            if (this.autoAtkEnabled) this.autoAtkLockInstance = target;
        ...
        if (
            this.map.isDoor(x, y) ||
            (this.inCombat() && !this.isRanged() && this.hasTarget() && !this.autoAtkEnabled)
        )
            return;
    ...
    public handleMovementStarted(x: number, y: number, speed: number, target: string): void {
        ...
        if (!target && (!this.autoAtkEnabled || !this.autoAtkLockInstance)) this.combat.stop();
    ...
    public handleMovementStep(
        ...
        if (this.autoAtkEnabled) this.tryPickupGroundAt(x, y);

        this.tryVipAutoAttack();

        this.lastStep = Date.now() - latency;
    }

    /**
     * Picks up items or opens loot bags on the tile the player is standing on.
     * Used when /autoatk keeps combat lock in movement packets so the client
     * may report the mob as target instead of ground loot.
     * @param x Grid x coordinate to check.
     * @param y Grid y coordinate to check.
     */

    private tryPickupGroundAt(x: number, y: number): void {
        if (!this.map.grids.hasEntityAt(x, y)) return;

        this.map.grids.forEachEntityAt(x, y, (entity) => {
            if (entity.instance === this.instance) return;

            if (entity.isLootBag()) {
                if (!entity.isOwner(this.username))
                    return this.notify(
                        `This lootbag belongs to ${Utils.formatName(entity.owner)}.`
                    );

                if (this.activeLootBag !== entity.instance) entity.open(this);
            ...
            if (entity.isItem()) {
                ...
                if (entity.owner === this.username)
                    this.statistics.addDrop(entity.key, entity.count);
    ...
    public handleMovementStop(x: number, y: number, target: string, orientation: number): void {
        // Client may send stop to sync position (e.g. trade) without a prior started packet.
        if (!this.moving) {
            let diffX = Math.abs(this.x - x),
                diffY = Math.abs(this.y - y);
    ...
    public removePet(): boolean {
        ...
        let petItemKey = Item.getPetItemKey(this.pet!.key),
            added = this.inventory.add(new Item(petItemKey, -1, -1, false, 1));
    ...
    public getSpawn(): Position {
        ...
        if (!this.quests.isTutorialFinished()) {
            if (this.map.tutorialSpawn) return { ...this.map.tutorialSpawn };

            if (this.map.spawn) return { ...this.map.spawn };
    ...
    /**
     * @returns Whether the account has an active VIP subscription.
     */

    public isVip(): boolean {
        return this.vipExpiresAt > Date.now();
    }

    public canUseAutoAtk(): boolean {
        return canUseVipAutoAtk(this.vipTier, this.isVip());
    }

    /**
     * When /autoatk is on, starts or maintains combat if a mob is within weapon range.
     */

    public tryVipAutoAttack(): void {
        if (!this.autoAtkEnabled || !this.canUseAutoAtk()) return;

        if (this.autoAtkLockInstance) {
            let locked = this.entities.get(this.autoAtkLockInstance);

            if (!locked?.isCharacter() || locked.dead) {
                this.releaseAutoAtkLock();

                return;
            }

            if (!this.canAttack(locked)) {
                this.releaseAutoAtkLock();

                return;
            }

            if (this.shouldReleaseAutoAtkLock(locked)) {
                this.releaseAutoAtkLock();

                return;
            }

            if (!this.combat.started || this.target?.instance !== locked.instance)
                this.handleTargetAttack(locked.instance, locked.x, locked.y);

            return;
        }

        let mob = this.findNearestMobInAttackRange();

        if (!mob) return;

        this.handleTargetAttack(mob.instance, mob.x, mob.y);
    }

    private releaseAutoAtkLock(): void {
        this.autoAtkLockInstance = '';
        this.target = undefined;
        this.combat.stop();
    }
    ...
    /**
     * Finds the closest attackable mob within the player's current weapon range.
     */

    private findNearestMobInAttackRange(): Character | undefined {
        let nearest: Character | undefined,
            nearestDistance = Infinity,
            searchRadius = this.isRanged() ? this.attackRange + (this.moving ? 1 : 0) : 1;

        this.forEachNearbyCharacter((entity) => {
            if (!entity.isMob() || entity.dead || !this.canAttack(entity)) return;
            if (!this.isInAttackRangeOf(entity)) return;

            let distance = this.getDistance(entity);
    ...
    /**
     * Ranged weapons drop lock when leaving attack range. Melee keeps lock while
     * combat is chasing a previously locked target within pursuit distance.
     */

    private shouldReleaseAutoAtkLock(target: Character): boolean {
        if (this.isRanged()) return !this.isInAttackRangeOf(target);

        if (this.isInAttackRangeOf(target)) return false;

        let distance = this.getDistance(target);

        if (
            (this.moving ||
                (this.combat.started && this.target?.instance === target.instance)) &&
            distance <= 12
        )
            return false;
    ...
    /**
     * Whether a character is within this player's weapon attack range.
     */

    private isInAttackRangeOf(target: Character): boolean {
        if (this.isRanged()) {
            ...
            return (
                this.getDistance(target) <= range && this.plateauLevel >= target.plateauLevel
            );
        }

        return this.isAdjacent(target);
    }

    /**
     * Combat XP multiplier from VIP loyalty tier (1 = no bonus).
     */

    public getVipExperienceMultiplier(): number {
        if (!this.isVip()) return 1;

        return getVipExperienceMultiplier(this.vipTier);
    }

    /**
     * Recomputes VIP tier months, syncs rank, and clears streak on expiry.
     */

    public refreshVip(): void {
        let now = Date.now();

        if (!this.isVip()) {
            if (isVipRank(this.rank)) this.setRank(Modules.Ranks.None);

            this.vipTier = 0;
            this.vipTierPeriodStart = 0;

            if (this.autoAtkEnabled) {
                this.autoAtkEnabled = false;
                this.releaseAutoAtkLock();
                this.send(
                    new CommandPacket({ command: 'autoatk', enabled: false })
                );
            }
        ...
        while (this.vipTier < VIP_MAX_TIER && now - this.vipTierPeriodStart >= VIP_MONTH_MS) {
            this.vipTier++;
            this.vipTierPeriodStart += VIP_MONTH_MS;
        }

        this.syncRankFromVip();
    }

    /**
     * Grants or extends VIP. Lapsed accounts restart at tier 0.
     * @param days Subscription length to add.
     * @param tier Optional loyalty tier (0–7) for admin/testing overrides.
     */

    public grantVip(days: number, tier?: number): void {
        let now = Date.now(),
            duration = days * 86_400_000,
            wasActive = this.vipExpiresAt > now;

        if (wasActive) this.vipExpiresAt += duration;
        else {
            this.vipExpiresAt = now + duration;

            if (tier === undefined) {
                this.vipTier = 0;
                this.vipTierPeriodStart = now;
            }
        }

        if (tier !== undefined) {
            this.vipTier = Math.max(0, Math.min(VIP_MAX_TIER, tier));
            this.vipTierPeriodStart = now;
        }
    ...
    /**
     * Founder pack VIP: extends active subscriptions and never lowers tier.
     * @param days Days to add (typically 30).
     * @param packTier Minimum loyalty tier granted by the pack (1, 3, 5, or 7).
     */

    public grantFounderPackVip(days: number, packTier: number): void {
        let now = Date.now(),
            duration = days * 86_400_000,
            wasActive = this.isVip(),
            clampedPackTier = Math.max(0, Math.min(VIP_MAX_TIER, packTier)),
            previousTier = this.vipTier;
        ...
        let newTier = Math.max(previousTier, clampedPackTier);

        if (newTier > this.vipTier) {
    ...
    /**
     * Removes VIP and resets loyalty tier streak.
     */

    public revokeVip(): void {
        this.vipExpiresAt = 0;
        this.vipTier = 0;
        this.vipTierPeriodStart = 0;
        this.refreshVip();
    ...
    private syncRankFromVip(): void {
        if (!this.isVip()) return;

        if (this.isMod() || this.isAdmin() || this.isHollowAdmin()) return;

        let rank = vipTierToRank(this.vipTier);
    ...
    private getVipBenefitTier(): number {
        if (this.isVip()) return this.vipTier;

        let legacy = rankToVipTier(this.rank);
     ...
     * depending on their rank. VIP tiers and administrators have a longer timeout
     * duration than regular players.
     */

    public getTimeoutByRank(): number {
        let vipTier = this.getVipBenefitTier();
    ...
    /**
     * Splash radius for the equipped melee weapon (tiles around the player). Ranged and magic
     * weapons do not use this property.
     */

    public getMeleeSplash(): number {
        if (this.isRanged() || this.isArcher() || this.isMagic()) return 0;

        return this.equipment.getWeapon().splash;
    }
    ...
    public chat(message: string, global = false, withBubble = true, colour = ''): void {
        ...
        if (this.isVip() && !this.isAdmin() && !this.isMod() && !this.isHollowAdmin()) {
            name = `[${getVipTitle(this.vipTier)}] ${name}`;

            if (global) colour = 'rgba(191, 161, 63, 1.0)';
            else {
                let vipColour = getVipChatColour(this.vipTier);

                if (vipColour) colour = vipColour;
                else if (this.vipTier > 0) colour = Modules.RankColours[Modules.Ranks.Patron];
            }
    ...
    public hasBattlePassClaim(track: BattlePassTrack, level: number): boolean {
        let key = getBattlePassClaimKey(track, level),
            legacy = getLegacyBattlePassClaimKey(track, level);

        return this.battlePassClaims.includes(key) || this.battlePassClaims.includes(legacy);
    }

    public claimBattlePass(track: BattlePassTrack, level: number): void {
        let key = getBattlePassClaimKey(track, level);

        if (this.hasBattlePassClaim(track, level)) return;

        this.battlePassClaims.push(key);
    ...
    public getLastAttackStyle(weaponType: string): Modules.AttackStyle {
        return Utils.normalizeAttackStyle(this.lastStyles[weaponType] ?? Modules.AttackStyle.None);
    }
