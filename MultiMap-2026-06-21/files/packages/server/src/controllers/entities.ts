...
export default class Entities {
    ...
    public constructor(private world: World) {
        for (let map of world.maps.getAll()) this.loadMapEntities(map);
        ...
        setInterval(
            () =>
                this.forEachMob((mob) => {
                    let map = this.getEntityMap(mob);

                    if (
                        !map.regions.get(mob.region)?.hasPlayersInRegion() &&
    ...
    /**
     * Spawns static entities and chests for a single map instance.
     */

    private loadMapEntities(map: Map): void {
        map.forEachEntity((position: Position, key: string) => {
            let type = this.getEntityType(key);

            switch (type) {
                case Modules.EntityType.Item: {
                    return this.spawnItem(key, position.x, position.y, false, 1, {}, '', map);
                }

                case Modules.EntityType.NPC: {
                    return this.spawnNPC(key, position.x, position.y, map);
                }

                case Modules.EntityType.Mob: {
                    return this.spawnMob(key, position.x, position.y, false, map);
                }

                case Modules.EntityType.Tree: {
                    return this.spawnTree(key, position.x, position.y, map);
                }

                case Modules.EntityType.Rock: {
                    return this.spawnRock(key, position.x, position.y, map);
                }

                case Modules.EntityType.FishSpot: {
                    return this.spawnFishSpots(key, position.x, position.y, map);
                }

                case Modules.EntityType.Foraging: {
                    return this.spawnForaging(key, position.x, position.y, map);
                }
            }
        });

        for (let info of map.chest)
            this.spawnChest(info.items?.split(',') || [], info.x, info.y, true, info.mimic, map);
    }

    /**
     * Resolves which map an entity belongs to.
     */

    private getEntityMap(entity: Entity): Map {
        if (entity.isPlayer()) return (entity as Player).map;

        if (entity instanceof Character) return entity.map;

        if (entity.entityMap) return entity.entityMap;

        return this.world.maps.getDefault();
    }
    ...
    public spawnItem(
        ...
        this.addItem(new Item(key, x, y, dropped, count, enchantments, owner), map);
    }
    ...
    public spawnMob(key: string, x: number, y: number, plugin = false, map?: Map): Mob {
    ...
    public spawnTree(key: string, x: number, y: number, map?: Map): Tree {
    ...
    public spawnRock(key: string, x: number, y: number, map?: Map): Tree {
    ...
    public spawnFishSpots(key: string, x: number, y: number, map?: Map): Tree {
    ...
    public spawnForaging(key: string, x: number, y: number, map?: Map): Tree {
    ...
    public spawnChest(
        ...
        chest.onOpen((player?: Player) => {
            ...
            if (player && mimic) {
                let mimicMob = this.spawnMob('mimic', chest.x, chest.y, false, map);

                // Mimic's death respawns the chest. We also ensure mimic doesn't respawn.
                if (mimicMob) {
                    mimicMob.respawnable = false;
                    mimicMob.chest = chest;
                }
            }
            ...
            this.spawnItem(item.key, chest.x, chest.y, true, item.count, {}, '', map);
        });

        this.addChest(chest, map);
    ...
    private spawnNPC(key: string, x: number, y: number, map?: Map): void {
        this.addNPC(new NPC(key, x, y), map);
    }
    ...
    public spawnProjectile(
        ...
        let projectile = new Projectile(owner, target, hit, projectileKey);
        ...
        this.add(projectile, owner.isPlayer() ? (owner as Player).map : undefined);
    ...
    private add(entity: Entity, map?: Map): void {
        ...
        let targetMap = map ?? this.getEntityMap(entity);

        entity.entityMap = targetMap;

        if (entity instanceof Character) entity.map = targetMap;

        targetMap.regions.handle(entity);
        targetMap.grids.addToEntityGrid(entity);
    }
    ...
    public addItem(item: Item, map?: Map): void {
        ...
        } else item.onRespawn(() => this.addItem(item, item.entityMap));
    ...
    public addMob(mob: Mob, map?: Map): void {
        this.add(mob, map);

        this.mobs[mob.instance] = mob;

        mob.addToChestArea(this.getEntityMap(mob).getChestAreas());
    }
    ...
    private addChest(chest: Chest, map?: Map): void {
        this.add(chest, map);
    ...
    private addNPC(npc: NPC, map?: Map): void {
        this.add(npc, map);
    ...
    public remove(entity: Entity): void {
        let map = this.getEntityMap(entity);
        ...
        map.grids.removeFromEntityGrid(entity);

        // Remove the entity from the region it is in.
        map.regions.remove(entity);
