...
import { BATTLE_PASS } from '@kaetram/common/config/battlepass';

import type { AccountBattlePassData } from '@kaetram/common/types/account-battlepass';
...
export interface PlayerInfo {
    ...
    /** Epoch ms when VIP subscription ends (0 = inactive). */
    vipExpiresAt?: number;
    /** Loyalty tier 0–7; resets to 0 if VIP lapses before renewal. */
    vipTier?: number;
    /** Epoch ms when the current VIP month period started (tier bumps every 30 days). */
    vipTierPeriodStart?: number;
    guild: string;
    pet: string;
...
export default class Creator {
    ...
    private saveTasks(player: Player): void {
        let collection = this.database.collection('player_tasks');

        this.updateCollection(collection, player.username, player.tasks.serialize());
    }
    ...
    /**
     * Saves battle pass progress for the whole account (email), not per character.
     */

    public saveBattlePass(player: Player): void {
        let accountKey = Loader.getAccountKey(player),
            collection = this.database.collection<AccountBattlePassData>('account_battlepass');
        ...
        collection
            ...
            .then((result) => {
                if (!result)
                    log.error(`Unable to save account_battlepass for ${accountKey}.`);
            })
            .catch((error) => {
                log.error(`An error occurred while saving account_battlepass for ${accountKey}.`);
                log.error(error);
            });
    }
    ...
    private saveCosmos(player: Player): void {
        let collection = this.database.collection('player_cosmos');

        this.updateCollection(collection, player.username, player.cosmos.serialize());
    }
    ...
    public static serialize(player: Player): PlayerInfo {
        return {
            ...
            vipTierPeriodStart: player.vipTierPeriodStart,
            guild: player.guild,
