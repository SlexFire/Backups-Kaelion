...
import {
    createBattlePassItem,
    getBattlePassReward,
    getBattlePassRewardMessage,
    isBattlePassTrack
...
import { setOdinHotbarSlotVipTiers } from '@kaetram/common/config/odinSkillsHotbar';
...
import type PlayerTask from '../game/entity/character/player/task/player-task';
...
export default class Commands {
    ...
    private async handlePlayerCommands(command: string, blocks: string[]): Promise<void> {
        switch (command) {
            ...
            case 'claimbp': {
                let track = blocks.shift(),
                    level = parseInt(blocks.shift()!);

                if (!isBattlePassTrack(track))
                    return this.player.notify('Esta recompensa ainda nao esta disponivel.');

                let reward = getBattlePassReward(track, level);

                if (!reward)
                    return this.player.notify('Esta recompensa ainda nao esta disponivel.');

                if (track === 'paid' && !this.player.hasBattlePassPremium())
                    return this.player.notify(
                        'Use o item Battle Pass para liberar a trilha premium.'
                    );

                if (!this.player.canClaimBattlePassReward(track, level))
                    return this.player.notify(
                        `Requer Lv - ${level} no Passe de Batalha (voce esta em Lv - ${this.player.getBattlePassLevel()}).`
                    );

                if (this.player.hasBattlePassClaim(track, level))
                    return this.player.notify('Voce ja coletou esta recompensa.');

                let item = createBattlePassItem(reward);

                if (!item.exists) return this.player.notify('Item de recompensa invalido.');

                if (this.player.inventory.add(item) < 1)
                    return this.player.notify('Sem espaco no inventario.');

                this.player.claimBattlePass(track, level);

                return this.player.notify(getBattlePassRewardMessage(reward));
            }
            ...
            case 'autoatk': {
                if (!this.player.isVip())
                    return this.player.notify('Auto-ataque VIP requer assinatura VIP ativa.');

                if (!canUseVipAutoAtk(this.player.vipTier, true)) {
                    let tiers = getVipAutoAtkTiers();

                    return this.player.notify(
                        tiers.length
                            ? `Auto-ataque disponivel nos tiers VIP: ${tiers.join(', ')}.`
                            : 'Seu tier VIP nao tem acesso ao auto-ataque.'
                    );
                }

                this.player.autoAtkEnabled = !this.player.autoAtkEnabled;

                if (!this.player.autoAtkEnabled) {
                    this.player.autoAtkLockInstance = '';
                ...
                this.player.send(
                    new CommandPacket({
                        command: 'autoatk',
                        enabled: this.player.autoAtkEnabled
                    })
                );

                return this.player.notify(
                    this.player.autoAtkEnabled
                        ? 'Auto-ataque VIP ativado. Entre no alcance de um monstro para atacar. Clique no monstro para soltar o alvo.'
                        : 'Auto-ataque VIP desativado.'
                );
            }
    ...
    private handleAdminCommands(command: string, blocks: string[]): void {
        ...
        switch (command) {
            ...
            case 'max': {
                let maxExperience = Formulas.LevelExp[Modules.Constants.MAX_LEVEL - 1];

                this.player.skills.forEachSkill((skill: Skill) => {
                    skill.setExperience(0);
                    skill.addExperience(maxExperience);
                });

                this.player.cosmos.grantAllCosmosMaxLevel();
            ...
            case 'resettasks': {
                this.player.tasks.forEachTask((task) => task.setStage(0));
            ...
            case 'finishtask': {
                taskKey = blocks.shift()!;

                if (!taskKey)
                    return this.player.notify(
                        `Malformed command, expected /finishtask taskKey`
                    );

                task = this.player.tasks.get(taskKey);

                if (task) task.finish();
                else this.player.notify(`Could not find task with key: ${taskKey}`);

                break;
            }

            case 'finishtasks': {
                return this.player.tasks.forEachTask((task) => task.finish());
            }
            ...
            case 'addodinskill': {
                ...
                if (!key) return this.player.notify(`Malformed command, expected /addodinskill key`);

                this.player.odinSkills.acquire(key);
                this.player.odinSkills.syncState(key);
                break;
            }
            ...
            case 'addcosmo': {
                ...
                if (!key) return this.player.notify(`Malformed command, expected /addcosmo key`);

                if (!this.player.cosmos.addCosmo(key))
                    return this.player.notify(`Could not add cosmo: ${key}`);
            ...
            case 'setodinskill': {
                ...
                if (!key || isNaN(x))
                    return this.player.notify(`Malformed command, expected /setodinskill key level`);

                if (x === 0 && !this.player.odinSkills.has(key))
                    this.player.odinSkills.add(key, 0);
                else if (!this.player.odinSkills.has(key) && x >= 1)
                    this.player.odinSkills.add(key, x);
                else this.player.odinSkills.setLevel(key, x);

                this.player.odinSkills.syncState(key);
                break;
            }

            case 'setodinhobarvip': {
                let slot3 = parseInt(blocks.shift()!),
                    slot4 = parseInt(blocks.shift()!),
                    slot5 = parseInt(blocks.shift()!);

                if ([slot3, slot4, slot5].some((tier) => isNaN(tier)))
                    return this.player.notify(
                        'Malformed command, expected /setodinhobarvip [tier slot3] [tier slot4] [tier slot5]'
                    );

                if ([slot3, slot4, slot5].some((tier) => tier < 0 || tier > VIP_MAX_TIER))
                    return this.player.notify('Tiers must be between 0 and 7.');

                setOdinHotbarSlotVipTiers([null, null, slot3, slot4, slot5]);

                return this.player.notify(
                    `Odin hotbar VIP: 3=T${slot3}, 4=T${slot4}, 5=T${slot5}. 1–2 livres para todos.`
                );
            }
            ...
            case 'setvip': {
                let days = parseInt(blocks.shift()!);

                username = blocks.join(' ');

                if (!username || isNaN(days))
                    return this.player.notify(
                        'Malformed command, expected /setvip [days] [username] (0 days = revoke)'
                    );

                player = this.world.getPlayerByName(username);

                if (!player) {
                    this.world.database.setVip(username, days);

                    return this.player.notify(
                        days <= 0
                            ? `VIP revoked for ${username} (offline).`
                            : `VIP updated for ${username} (+${days} days, offline).`
                    );
                }
                ...
                if (days <= 0) {
                    ...
                    return this.player.notify(`VIP revoked for ${username}.`);
                }

                player.grantVip(days);

                return this.player.notify(
                    `VIP active for ${username} until ${new Date(player.vipExpiresAt).toLocaleString()} (tier ${player.vipTier}).`
                );
            }

            case 'setautoatktiers': {
                if (blocks.length === 0)
                    return this.player.notify(
                        `Tiers com /autoatk: ${getVipAutoAtkTiers().join(', ') || 'todos (lista vazia)'}. Uso: /setautoatktiers [0-7 ...]`
                    );

                let tiers: number[] = [];

                for (let block of blocks) {
                    let tier = parseInt(block);

                    if (isNaN(tier) || tier < 0 || tier > VIP_MAX_TIER)
                        return this.player.notify(
                            `Tier invalido: ${block}. Use valores de 0 a ${VIP_MAX_TIER}.`
                        );

                    tiers.push(tier);
                }

                setVipAutoAtkTiers(tiers);

                return this.player.notify(
                    tiers.length
                        ? `/autoatk liberado para tiers VIP: ${getVipAutoAtkTiers().join(', ')}.`
                        : '/autoatk liberado para todos os tiers VIP ativos.'
                );
            }

            case 'setviptier': {
                let tier = parseInt(blocks.shift()!),
                    days = parseInt(blocks.shift()!);

                username = blocks.join(' ');

                if (!username || isNaN(tier) || isNaN(days))
                    return this.player.notify(
                        'Malformed command, expected /setviptier [0-7] [days] [username]'
                    );

                if (tier < 0 || tier > 7)
                    return this.player.notify('Tier must be between 0 and 7.');

                player = this.world.getPlayerByName(username);

                if (!player) {
                    if (days <= 0) {
                        this.world.database.setVip(username, 0);

                        return this.player.notify(`VIP revoked for ${username} (offline).`);
                    }

                    this.world.database.setVip(username, days, tier);

                    return this.player.notify(
                        `VIP tier ${tier} (+${days} days) set for ${username} (offline).`
                    );
                }
                ...
                player.grantVip(days, tier);
                player.save();

                return this.player.notify(
                    `VIP tier ${tier} for ${username} until ${new Date(player.vipExpiresAt).toLocaleString()} (${getVipTitle(tier)}).`
                );
            }
            ...
            case 'settime': {
                ...
                if (!period)
                    return this.player.notify(
                        'Uso: /settime [morning|afternoon|evening|night|deep_night|manha|tarde|entardecer|noite|madrugada]'
                    );
