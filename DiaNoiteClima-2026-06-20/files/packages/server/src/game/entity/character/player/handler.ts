...
import type SkillsStructureOdin from './odin_skill/skills_structure_odin';
...
export default class Handler {
    ...
    public constructor(private player: Player) {
        ...
        this.player.tasks.onLoaded(this.handleTasks.bind(this));
        this.player.skills.onLoaded(this.handleSkills.bind(this));
        this.player.abilities.onLoaded(this.handleAbilities.bind(this));
        this.player.odinSkills.onLoaded(this.handleOdinSkills.bind(this));
        this.player.cosmos.onLoaded(this.handleCosmos.bind(this));
        this.player.odinSkills.onAdd(this.handleOdinSkillAdd.bind(this));
        this.player.odinSkills.onCast(this.handleOdinSkillCast.bind(this));
    ...
    private handleDoor(door: ProcessedDoor): void {
        ...
        if (door.hasInteriorToggle) this.player.syncEnvironmentFromPosition();
    ...
    private handleMovement(x: number, y: number): void {
        ...
        this.player.syncEnvironmentFromPosition();
    ...
    private handleEquip(equipment: Equipment): void {
        ...
        if (equipment.type === Modules.Equipment.Weapon)
            this.player.odinSkills.grantStaffSkills();

        this.player.odinSkills.syncState();
    ...
    private handleTasks(): void {
        this.player.send(
            new TaskPacket(
                Opcodes.Task.Batch,
                this.player.tasks?.serialize(true)
            )
        );
    }
    ...
    /**
     * Envia batch de skills Odin ao cliente.
     */
    private handleOdinSkills(): void {
        this.player.send(
            new OdinSkillPacket(Opcodes.OdinSkill.Batch, this.player.odinSkills?.serialize())
        );
    }

    private handleCosmos(): void {
        this.player.send(
            new CosmoPacket(Opcodes.Cosmo.Batch, this.player.cosmos.serialize())
        );
    }

    /**
     * Envia uma nova skill Odin ao cliente.
     */
    private handleOdinSkillAdd(skill: SkillsStructureOdin): void {
        this.player.send(
            new OdinSkillPacket(Opcodes.OdinSkill.Add, skill.serialize(this.player))
        );
    }

    /**
     * Notifica o cliente sobre o efeito visual de uma skill Odin.
     */
    private handleOdinSkillCast(
        key: string,
        x: number,
        y: number,
        areaRadius: number,
        ...
        this.player.sendToRegions(
            new OdinSkillPacket(Opcodes.OdinSkill.Cast, {
                key,
                x,
                y,
                areaRadius,
    ...
    private handleKill(character: Character): void {
        ...
        // Progress all active tasks associated with this mob kill.
        this.player.tasks.checkResets();

        this.player.tasks.forEachTask((task) => {
            if (!task.hasMob(character) || task.isClaimed()) return;

            task.killCallback?.(character);
        });
    }
